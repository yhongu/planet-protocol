...
const MATERIAL_LADDER: [string, string][] = [
  ...
  ["copper", "銅石器時代"],
  ["pottery", "新石器時代"],
  // ★**火は素材の段にしない**（2026-09-09）。三時代法は「何で道具を作るか」の
  //   区分で、火は道具の材料ではない。段にしていたので画面に
  //   **「旧石器時代（火）」と「旧石器時代」という、読んで区別できない 2 つ**が
  //   並んだ（★プレイして報告された）。火の効きは
  //   **エネルギーの軸**（White の法則。300W → それ以上）に出る
  ["stoneTools", "旧石器時代"],
]
