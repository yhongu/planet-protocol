...
/**
 * 同時に追跡する系統の数（バイオマスの場のレーン数）。
 *
 * ★**16 では足りなかった。** 実測（4 seed・全史）で **3 本が上限に張り付き**、
 * `docs/02` の「20〜50 程度」にも届いていなかった。
 * そして**これが進化の速さを決める** —— 新機能化の抽選は
 * 「クレードごと × 100 万年ごと」に走るので、**引く回数はクレード数に比例**する。
 *
 * ★**費用はほぼゼロ**（2026-09-06 実測、96x48・顕生代の章）:
 *   クレード 4 本 53.8ms/歩 / 16 本 52.7ms/歩 —— 差は測定誤差。
 *   1 歩の費用は**気候ソルバが支配**していて、生命のループは見えない。
 *
 * ★**レーン数を変えると場の大きさが変わる。** `SNAPSHOT_VERSION` を
 * 上げないと、古いセーブを読んだときに場が食い違って黙って壊れる。
 */
export const MAX_CLADES = 32
