...
export interface CarbonParams {
   ...
   *
   * ★★**2026-09-10: 4 seed の対応比較で測って【却下】した。既定は 0 のまま。**
   *
   * 予言どおり太古代は暖まる（4/4 seed）が、**その先が凍る**:
   *
   * | 4 seed 中央値 | CO2ppm | 気温C | 氷% | 4 seed の向き |
   * |---|---|---|---|---|
   * | 太古代 A → B | 5486 → 7520 | −2.2 → **−0.2** | 34.2 → 29.9 | 暖 4/4・氷減 4/4 ✓ |
   * | 原生代 A → B | 2231 → 1762 | 7.4 → **1.6** | 19.5 → 26.6 | 暖 0/4・氷増 4/4 ✗ |
   * | 顕生代 A → B | 642 → 704 | 16.7 → **3.2** | 6.1 → 25.7 | 暖 0/4・氷増 4/4 ✗ |
   *
   * ★**顕生代は CO2 がむしろ高いのに 13.5K 寒く、氷が 4 倍**——
   * CO2 の効果ではなく**氷アルベドの別の枝**に落ちている。
   * モデルで唯一きれいに地球へ合っていた点（顕生代 16.7℃ ≒ 地球 15℃）が壊れる。
   * ★日射の列が A/B で厳密に一致していることを確かめてある（カナリア・罠 13）。
   * ★単一ランの監査では 顕生代 14.4℃ と**壊れて見えなかった** ——
   * 1 本で「直った」と言わなくてよかった（罠 3）。
   *
   * ★★**根は分母にある。** `presentDayCrustProduction` は
   * 「**現在の厚さ（1350℃）× そのときの発散場**」であって、
   * **現在の地球の生産量ではない**。実測（96x48・seed g01）:
   *
   *   現在の地球で  実測の生産 21.2 km³/yr  /  分母に使う値 13.0  → **比 0.61**
   *
   * 基準状態ですら 39% 小さい。**分母と分子は同じ推定器で撮ること**（罠 14）。
   *
   * ★**次にやること**: 分母を「基準状態で、実測と同じ推定器で」撮る ——
   * 冥王代スタートでも**現在の地球の世界を別に組んで較正する**か、
   * **最初に可動蓋になって生産が実測できた歩で撮り直す**。
   * どちらも較正の定義（`assertReferenceState`）に関わるので、
   * 直したら顕生代 16.7℃ が動いていないことを 4 seed で確かめること。
   */
