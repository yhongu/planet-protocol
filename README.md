...
### ▶ [ブラウザで遊ぶ — gaia-protocol.pages.dev](https://gaia-protocol.pages.dev)

インストール不要。最初の読み込みは **168 KB**（計算はすべてブラウザの中で走ります）。
**「顕生代から」を選べば、生命の満ちた惑星から一瞬で始まります**
（14.2℃・CO₂ 710ppm・O₂ 15.7%・16 クレード）。
...
`MIT` ・ TypeScript + Vite ・ 物理テスト **167 件** ・ 総合監査 **30 項目**
...
**キー** — `1`〜`4` で介入を構える・`空白` で停止と再開・`,` `.` で速度の段・
`[` `]` でレイヤを送る・`Esc` で構えを解く。ボタンの隅にその文字が出ている。
...
- **レイヤ**（左下）— 25 枚。**分類と色見本つきのタイルで一度に全部見える**。
  惑星・標高・気温・氷・風化レジーム・河川・プレート・地殻年代・海底熱水・
  湧昇・生物量・優占クレード・多様性…
  ★**凡例に「絶対目盛りか相対か」が書いてある**（多くは相対）
- **年代記**（右下）— 起きたことの全部。生命 / 地質 / 気候 / 介入で絞れる
- **スパークライン** — CO₂・O₂・氷・陸・生物圏に線が付く。
  ★標本は**年**で取る（フレームではない）ので、速度の段やマシンの速さで
  線の形が変わらない
...
> 球体表示。正射図法（Snyder の逆変換）で、平面図と**同じバッファを標本化**して
> いるので、レイヤ側は 1 行も変えなくてよい。明暗は 4 段＋ディザ。

### ピクセル・ファースト

**見えているドットは全部、物理量から計算されている。**

混色は【段】に量子化してある。もとの実装は混色が連続で、1 枚に **18049 色**
あったので、拡大すると海岸線が溶けた。段にして **111 色**まで落とした。

★**量子化するのは【色】であって【場】ではない。** 標高も氷もバイリニアで
補間したまま色に落とすので、**海岸線はセル境界の階段ではなく 0 等高線の曲線**に
なる。ディザ（4x4 Bayer）は**汀線と氷縁だけ**に掛ける —— 海の深さのような
なだらかな傾きに掛けると、画面いっぱいが砂嵐になった。

★陸と海の重みだけは補間しない。1 セルがちょうど 4x4 の Bayer に重なるので、
**そのセルの陸の割合が 16 段のドットの密度**になり、面積が厳密に保たれる。

拡大率は「アート 1 ピクセル = 画面 n ピクセル」に丸め、原点をデバイス
ピクセルに揃える。**どの倍率でもドットが割れない。**

```bash
npx vite-node scripts/probes/probe-pixel.ts   # 3.5 秒。色数を印字する
```
...
出来事は**時刻を決め打ちしていない**。場から検出しているだけなので、
惑星ごとに違う年表が出る。seed `gaia-6` の場合:
...
★**2.20Ga に 3 つが並んでいる** —— 酸素発生型光合成の獲得 → 大酸化事変 →
大量絶滅。**酸素が嫌気性の生命を殺した瞬間**が、指定なしで自動的に出た。
別の seed（`audit`）では大酸化事変が 3.49Ga と早く、大量絶滅は起きず、
真核が 1.52Ga だった。
...
**動いているもの: https://gaia-protocol.pages.dev** —— 実測の内訳:

| | |
|---|---|
| 初回の読み込み | **168 KB**（html 5.9 + JS 50.6 + Worker 56.3 + CSS 7.2 + 絵 48） |
| 章を 1 つ選ぶと | +**15.4 MB**（配られた惑星のセーブ） |
| 運用費 | **¥0** —— 完全な静的サイトなので Workers も KV も使わない |

★**1 ファイル 25MiB の上限に注意。** 章は 96×48 で 15.4MB です。128×64 で作ると
約 26MB になって**デプロイが弾かれます**。解像度を上げるなら R2 へ移すか、
粒子の位置を float32 にする（約 30% 減）必要があります。
