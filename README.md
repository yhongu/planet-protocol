...
- 惑星を生成し、**45 億年を通して**気候・炭素・水・固体地球が結合して動く
- テクトニクス様式がマントル熱史から創発する（マグマオーシャン → … → モバイルリッド）
- 大陸が動き、衝突し、山ができ、川が削る
- 風化サーモスタットが効いたり、供給律速で弱ったりする
- LIP が LLSVP の縁の通過で起きる（予測可能な災厄）
- **なぜ気温が / CO₂ が変わったかを寄与分解パネルで読める**
- タイムラインに出来事が刻まれ、介入（噴火・隕石・プレート・造山）が打てる

生命と文明はまだ無い（M5 / M6）。

### 45 億年通しても壊れない

`npx vite-node scripts/full-history.ts` で全史を通せる（64×32 で約 3 分）。

| | 45 億年後 |
|---|---|
| 陸地面積 | **35.1%**（29% で開始、超大陸サイクルに合わせて振動） |
| 大陸地殻の体積 | **1.20**（緩やかに成長して飽和） |
| テクトニクス様式の遷移 | 7 回（マグマオーシャン → … → モバイルリッド） |
| LIP | **20 回**（地球は 20〜30 回） |
| 数値のクランプ | **0 回** |

ここに至るまでに収支の構造的な誤りを 4 つ潰した。
経緯は [docs/05](docs/05-roadmap.md) —— **当てずっぽうでパラメータを動かして
5 回外したあと、地殻の厚さに寄与台帳を付けて測ったら一発で分かった**話。
...
npm install
npm run dev        # 開発サーバ
npm test           # 検証テスト (29 項目)
npm run build      # 型チェック + プロダクションビルド

npx vite-node scripts/snapshot.ts   # ブラウザ無しで生成結果を PNG 出力 (snapshots/)
```
...
フェーズ2 で WebGPU compute にホットカーネルを移植する（[docs/04 §8](docs/04-architecture.md)）。
**移植可能な形で書くための規則は初日から守っている**（SoA / ping-pong / gather / 固定順序リダクション）。
